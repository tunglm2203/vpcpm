...
class CutoutColor(object):
    ...
    def do_augmentation(self, imgs):
        ...
        for i, (img, w11, h11) in enumerate(zip(imgs, self.w1, self.h1)):
            cut_img = img.copy()
            for ind in range(cut_img.shape[0]//3):
                cut_img[ind*3: (ind+1)*3, self.pivot_h+h11:self.pivot_h+h11+h11, self.pivot_w+w11:self.pivot_w+w11+w11] \
...
class ColorJitter(nn.Module):
    ...
    def forward(self, inputs):
        ...
        if random_inds.sum() > 0:
            for ind in range(inputs.shape[1]//3):
                inputs[inds,ind*3:(ind+1)*3, : , :] = self.transform(inputs[inds,ind*3:(ind+1)*3, : , :])
        return inputs
